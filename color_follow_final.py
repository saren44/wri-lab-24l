...
statesDict = {
    ...
    10: lambda r, c: r.color_turn(),
    11: lambda r, c: r.follow([BLACK, c]),
    12: lambda r, c: r.release_object(),
    13: lambda r, c: r.follow([BLACK, c]),
...
class LineFollower:
    ...
    def follow(self, followed=[BLACK]):
        if (self.left_color not in followed and self.right_color not in followed):
            ...
            tank_drive.on(SpeedPercent(self.speed / 5), SpeedPercent(self.speed / 5))
        elif (self.left_color in followed and self.right_color not in followed):
            tank_drive.on(SpeedPercent(-self.speed / 3), SpeedPercent(self.speed / 3))
    ...
    def release_object(self):
        tank_drive.on_for_rotations(SpeedPercent(self.speed), SpeedPercent(self.speed), 0.8)
        hand.on_for_rotations(SpeedPercent(self.hand_speed), 0.25)
        tank_drive.on_for_rotations(SpeedPercent(-self.speed), SpeedPercent(-self.speed), 0.55)
        sleep(1)
        self.rotate()
        self.state = 13
